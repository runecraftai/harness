...
| D11 | **README raiz via skill beautify-github-readme (DOC-06)**: instalar a skill (`npx skills add oil-oil/beautify-github-readme`); executar no modo escolhido pelo usuário (QA-7 — default Whole README sobre o conteúdo do T11); a skill decide o sistema visual lendo o repo (prova real); **preview local SEMPRE antes de commit/push** (regra da skill); zero invenção de capacidades (F8); assets GitHub-safe (SVG editável como fallback estático; GIF só opt-in); escopo = README raiz; umbrella opcional por decisão do usuário no preview | A camada de conteúdo do T11 permanece (a skill reorganiza se necessário); o visual é derivado do projeto, não template genérico; aprovação explícita é gate de commit |
...
## Open questions para o usuário (QA-4..7 — confirmar antes do Execute)
...
- **QA-7 — skill beautify-github-readme (modo + escopo)**: **(a) recomendado — Whole README sobre o README raiz** (a skill redesenha o conteúdo do T11 com o sistema visual completo; preview local antes de commit) · (b) Asset-only (só hero SVG/headers/badges; copy do T11 intacto) · (c) Whole README raiz + umbrella (2 READMEs; maior escopo)
