...
**Current Work:** **M7 — F24 Execution Guards COMPLETE** (guards do guild como extensões Pi com bloqueio real `{ block: true }`; 398 testes; cleric APPROVE + 1 fix). **Próximo: F25 Verification Cascade** (P1 desbloqueada por AD-022; judge LLM env-gated). F22 continua **aguardando aprovação explícita** (custo de tokens); F23 P1 desbloqueada (AD-022). Roadmap com M7 (Garantias) e M8 (Multi-Agent) — ver AD-008..011, AD-016..021, AD-022 (decisões de escopo M7/M8).
