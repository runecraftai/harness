...
**Current Work:** **M4 COMPLETE — F20 COMPLETE** (receipt leve + delivery gates: capture RPC/--from, hooks pre-commit/pre-push fail-closed, kill switch, check 17; **330 testes no harness**). **M4 FECHADO (F19, F20).** Próximo: M5 (F21 suite determinística → F22 E2E → F23 ratchets).** M1/M2/M3 completos. Roadmap reestruturado (M2–M6) — ver AD-008..011; decisões de execução do M2 em AD-016; F17 em AD-017; F18 em AD-018; F19 em AD-019; F20 em AD-020.
...
- **Next:** M5 — F21 (suite determinística com fixture de modelo) → F22 (cenários E2E versionados) → F23 (ratchet baselines). Specs/designs prontos e revisados (2026-08-05, AD-015).
