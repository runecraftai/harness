/**
 * E2E: prove a Claude Code user can reach taskflow through MCP.
 *
 * Spawns the real bin.ts as a stdio MCP server (exactly as `claude mcp add`
 ...
 * proof. The fully-manual half (Claude Code itself invoking the tool) is:
 *
 *   claude mcp add taskflow -- node --experimental-strip-types <pkg>/src/mcp/bin.ts
 *   claude -p 'Call the taskflow_list MCP tool and report what it returns.'
 *   → claude emits an mcp tool call for server "taskflow" tool "taskflow_list"
 *     and relays the saved-flow list. (then: claude mcp remove taskflow)
 *
 * Run: node --experimental-strip-types test/e2e-claude-mcp.mts
 */
...
console.log("▶ launching claude-taskflow MCP server (as Claude Code would) …\n");
...
console.log("\n✅ E2E PASS — claude-taskflow serves MCP over stdio; Claude Code can discover and call its tools.");
process.exit(0);
