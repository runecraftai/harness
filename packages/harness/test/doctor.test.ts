...
describe("doctor — pass e read-only (LIFE-01)", () => {
  ...
  test("--json reporta checks e summary parseáveis", async () => {
    ...
    try {
      ...
      expect(json.checks).toHaveLength(20); // 1,2,3,5,6 (F12) + 7–15 (F18) + 16 (F19 driver) + 17 (F20 gates) + 18 (F24 guards) + 19 (F25 verification) + 20 (F30 models) + 21 (F31 copilot)
      expect(json.summary.pass + json.summary.warn).toBe(15); // check 20 (Models) = warn (models.json ausente no sandbox); check 21 (copilot) = pass
...
describe("doctor — warns (colisão) e scopes", () => {
  ...
  test("scope workspace: state e settings do projeto considerados (edge -l)", async () => {
    ...
    try {
      ...
      expect(summaryLine(result.stdout)).toContain("pass 14"); // 1,2,3,5,6 + 7,12,14,16 (informativos) + 17 (gates) + 18 (guards) + 19 (verification) + 21 (copilot — não detectado)
    } finally {
      sb.cleanup();
    }
  });
