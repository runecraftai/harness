...
| EVAL-015 | baseline-diff vs ratchet F23 (F26 — implementado; reservado no arcanum) | eval (framework) | 1. run normal → no-regression (pass); 2. falha nova vs baseline → regression (fail com reason caseId + mensagem normalizada); 3. falha congelada → pass (informacional); 4. baseline ausente → degraded informacional (não falha infra) | reusa normalizeMessage/sortLines/parseBaselineLines do F23 (sem duplicação); identidade 2-partes `caseId<TAB>mensagemNormalizada` (namespace F26) |
