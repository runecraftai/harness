...
| **Memory** (F29) | persistent cross-session memory: SQLite+FTS5 (bun:sqlite no Bun; node:sqlite no Node — F35), 10 `rune_*` tools, `using-runes` skill | `memory` in state.json; `harness memory`; [MEMORY.md](MEMORY.md); `RUNECRAFT_MEMORY=0` |
