...
- **Capability manifest (B0)** — `packages/harness/src/capabilities/manifest.ts` is the single source of truth for per-agent capability claims (hooks/subagents/mcp/models/guards + taskflow/goal-loop/pr-review/memory/persona/routing/sdds). `matrix.ts` unsupported-cell reasons, `doctor` check 25 (digest) and `status` (Capabilities section) all consume it. Future parity phases (B2..B8, PARITY.md roadmap) update the manifest FIRST; the digest golden test catches drift.
